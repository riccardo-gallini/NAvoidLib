...
namespace NAvoidTest {
	...
	int test2::main(void)
	{
		...
		//new Avoid::ShapeConnectionPin(shapeRef3, INPUT, 0.25, 0.4, true, 0, Avoid::ConnDirLeft);
		//new Avoid::ShapeConnectionPin(shapeRef3, INPUT, 0.75, 0.6, true, 0, Avoid::ConnDirLeft);
		Avoid::ShapeConnectionPin *pin = new Avoid::ShapeConnectionPin(shapeRef3, INPUT, 0.0, 0.5, true, 0, Avoid::ConnDirLeft);
		pin->setExclusive(false);
		//new Avoid::ShapeConnectionPin(shapeRef3, INPUT, 0.0, 0.5, true, 0, Avoid::ConnDirLeft);
		

		// And centre pins for two other shapes.
		new Avoid::ShapeConnectionPin(shapeRef1, Avoid::CONNECTIONPIN_CENTRE, Avoid::ATTACH_POS_CENTRE, Avoid::ATTACH_POS_CENTRE, true, 0.0, Avoid::ConnDirNone);
		new Avoid::ShapeConnectionPin(shapeRef2, Avoid::CONNECTIONPIN_CENTRE, Avoid::ATTACH_POS_CENTRE, Avoid::ATTACH_POS_CENTRE, true, 0.0, Avoid::ConnDirNone);
		...
		router->outputDiagramSVG("c:\connectionpin02-1");

		router->moveShape(shapeRef1, 0, 90);
		router->moveShape(shapeRef2, 0, -90);

		router->processTransaction();
		router->outputDiagramSVG("c:\connectionpin02-2");

		router->moveShape(shapeRef3, 0, -60);

		router->processTransaction();
		router->outputDiagramSVG("c:\connectionpin02-3");
