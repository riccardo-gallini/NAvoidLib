...
ConnEnd::ConnEnd()
    : m_type(ConnEndEmpty),
      ...
      m_conn_ref(NULL),
      m_active_pin(NULL),
	  m_orientation(ConnEndNotOriented)
{
}
...
std::vector<Point> ConnEnd::possiblePinPoints(void) const
{
    ...
    return m_anchor_obj->possiblePinPoints(m_connection_pin_class_id, m_orientation);
}
...
// for this pinClassId.
// RG 2016
void ConnEnd::assignPinVisibilityTo(VertInf *dummyConnectionVert, VertInf *targetVert, ConnEndOrientation wanted_orientation)
{
            ...
            curr != m_anchor_obj->m_connection_pins.end(); ++curr)
    {
        ...
        if ((currPin->m_class_id == m_connection_pin_class_id) && 
                currPin->isPossiblePin(wanted_orientation))
        {
...
//RG 2016
ConnEndOrientation ConnEnd::Orientation() {
	return m_orientation;
}
void ConnEnd::SetOrientation(ConnEndOrientation orientation) {
