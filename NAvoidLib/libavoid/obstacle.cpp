...
//RG 2016 - modified for considering the exclusive by orientation for pins
std::vector<Point> Obstacle::possiblePinPoints(unsigned int pinClassId, ConnEndOrientation wanted_orientation) const
{
            ...
            curr != m_connection_pins.end(); ++curr)
    {
		...
		if (currPin->m_class_id == pinClassId) {
			...
			if (currPin->isPossiblePin(wanted_orientation)) {
