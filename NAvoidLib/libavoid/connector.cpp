...
std::pair<bool, bool> ConnRef::assignConnectionPinVisibility(const bool connect)
{
    ...
    if (dummySrc)
    {
        ...
        if (connect)
        {
            m_src_connend->assignPinVisibilityTo(m_src_vert, m_dst_vert, ConnEndOrientation::ConnEndSource);
        }
    }
    bool dummyDst = m_dst_connend && m_dst_connend->isPinConnection();
    if (dummyDst)
    {
        ...
        if (connect)
        {
            m_dst_connend->assignPinVisibilityTo(m_dst_vert, m_src_vert, ConnEndOrientation::ConnEndTarget);
        }
    }
