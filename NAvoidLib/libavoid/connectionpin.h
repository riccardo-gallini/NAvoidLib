...
// destructors and prevent generation of default destructors.
		...
		//! @brief  RG 2016 Sets whether the pin is exclusive by orientation, i.e., only connectors
		//!         with the same orientation (incoming/outgoing) can attach to it.  
		//!			This is weaker than "exclusive", so that if the pin is exclusive this will not be considered.
		//!
		//! @param[in]  exclusive  A bool representing whether this pin should
		//!                        be exclusive by orientation
		//!
		void setExclusiveByOrientation(const bool exclusive);

		//! @brief RG 2016 Returns whether the connection pin is exclusive by orientation, 
		//!        i.e., only connectors with the same orientation (incoming/outgoing) can attach to it.
		//!		   This is weaker than "exclusive", so that if the pin is exclusive this will not be considered.
		//!
		//! @return  A boolean denoting whether this pin is exclusive by orientation.
		//!
		bool isExclusiveByOrientation(void) const;

		//! @brief RG 2016  Returns the first used orientation for this pin (useful for checking the availability
		//!                 of this pin for exclusive by orientation.
		//! @return         The orientation used for this pin.
		ConnEndOrientation getUsedOrientation();

		//! @brief RG 2016  Returns true if this pin is usable taking into account its exclusiveness constraints.
		//!
		//! @param[in]  wanted_orientation    for exclusive by orientation pins.
		//! @return     true				  if this pin is available for routing
		bool isPossiblePin(ConnEndOrientation wanted_orientation);
    ...
    private:
		...
		// RG 2016
		bool m_exclusive_by_orientation;
};
