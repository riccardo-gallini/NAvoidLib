...
ShapeConnectionPin::ShapeConnectionPin(ShapeRef *shape, 
    ...
    : m_shape(shape),
	  ...
	  m_exclusive_by_orientation(false)
{
...
//RG 2016
bool ShapeConnectionPin::isExclusiveByOrientation(void) const {
	return m_exclusive_by_orientation;
}

void ShapeConnectionPin::setExclusiveByOrientation(const bool exclusive) {
	m_exclusive_by_orientation = exclusive;
}


//RG 2016
ConnEndOrientation ShapeConnectionPin::getUsedOrientation() {

	if (m_connend_users.empty()) {
		return ConnEndOrientation::ConnEndNotOriented;
	}
	else
	{
		ConnEnd *e = *(m_connend_users.begin());
...
//RG 2016
bool ShapeConnectionPin::isPossiblePin(ConnEndOrientation wanted_orientation) {

	if (this->m_exclusive) {
		//it's exclusive 
		if (this->m_connend_users.empty()) {
			//it's free => possible pin
			return true;
		}

	}
	else if (this->m_exclusive_by_orientation) {
		ConnEndOrientation orientation = this->getUsedOrientation();
		if (orientation == ConnEndOrientation::ConnEndNotOriented || orientation == wanted_orientation) {
			return true;
		}
	}
	else {
		//it's either exclusive neither exclusive by orientation
		//so it's a possible pin
		return true;
	}
